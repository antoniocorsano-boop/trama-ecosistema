#!/usr/bin/env python3
"""TRAMA-SA-01/R2: TypeSafe evidence-sufficiency gate + alignment.
...
DEFAULT_OUTPUT = ROOT / "artifacts" / "trama-sa01-typesafe-r2-raw.json"
...
def run_case(client, Choice, Noul, case: dict, model: str) -> dict:
    ...
    return {
        ...
        "judgmentDesign": "R2_EVIDENCE_THEN_ALIGNMENT",
...
def execute(output: Path, model: str) -> int:
    ...
    print(
        f"TRAMA-SA-01/R2 raw run: {len(results)} results, "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="TRAMA-SA-01/R2 TypeSafe lab adapter")
